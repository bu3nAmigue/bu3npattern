...
const PROGRAMS = {
    ...
    void main() {
        ...
        #endif
        vec4 videoInfo = texture2D(u_videotex,  xy/u_output.size *0.5 -vec2(0.,0.1));
        float ch = getOutputChannel();
        if (ch >= u_output.depth4)
            return;

        float filterBand = floor((ch+0.5)/u_input.depth4);
        float inputCh = ch-filterBand*u_input.depth4;//+videoInfo.r; //CHECCCCCC
        if (filterBand < 0.5) {
...
export class CA {
    ...
    updateVideoTex(video){
        const gl = this.gl;
        const texture = this.videoTex;
